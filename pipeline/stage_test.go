...
func (s StageTestSuite) TestFIFO(c *gc.C) {
	...
	for i := 0; i < len(stages); i++ {
		stages[i] = pipeline.FIFO(makePassthrouhProcessor())
	}
...
func (s StageTestSuite) TestFixedWorkerPool(c *gc.C) {
	...
	src := &sourceStub{data: stringPayloads(numWorker)}

	p := pipeline.New(pipeline.FixedWorkerPool(proc, numWorker))
...
func(s StageTestSuite)TestBroadcast(c *gc.C){
	...
	expData := []pipeline.Payload{
		&stringPayload{val: "0_0", proccessed: true},
		&stringPayload{val: "0_1", proccessed: true},
		&stringPayload{val: "0_2", proccessed: true},
	}
...
func makeMutatingProcessor(index int)pipeline.Processor{
	return pipeline.ProcessorFunc(func(ctx context.Context, p pipeline.Payload) (pipeline.Payload, error) {
...
func makePassthrouhProcessor() pipeline.Processor {
